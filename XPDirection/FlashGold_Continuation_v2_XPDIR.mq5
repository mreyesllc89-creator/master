...
//|                            the flip bar.                          |
//| The vote is therefore read from FAST/SLOW: fast > slow is the     |
//| bullish TDI state and votes BUY. See the report, OWNER_TO_CONFIRM |
//| XPDIR_POLARITY - with invertFill = true that bullish state is the |
//| panel's RED fill, not its LIME fill.                              |
//+------------------------------------------------------------------+
...
double   g_XPDirState[XPDIR_RUNGS];
...
// EMPTY_VALUE in FAST or SLOW is the map's valid flag (BufFast[i] = fastOk ?
// fast : EMPTY_VALUE). STATE is deliberately not consulted: it is isRedNow,
// which is 0 both for "not red" and for an invalid bar, and the map's
// invertFill input flips its polarity.
int XPDir_RungVote(const double fast, const double slow, const int runLen,
                   const long ageSeconds, const int intervalSeconds,
                   const int maxStaleBars, const int maxRunLenBars,
                   string &whyOut)
{
   if(!MathIsValidNumber(fast) || !MathIsValidNumber(slow) ||
      MathAbs(fast) >= EMPTY_VALUE || MathAbs(slow) >= EMPTY_VALUE)
   { whyOut = "map_invalid"; return 0; }
   if(ageSeconds > (long)maxStaleBars * (long)intervalSeconds)
   { whyOut = "stale"; return 0; }
   if(maxRunLenBars > 0 && runLen > maxRunLenBars)
   { whyOut = "runlen_capped"; return 0; }
   if(fast > slow) { whyOut = "ok"; return  1; }   // bullish TDI -> BUY
   if(fast < slow) { whyOut = "ok"; return -1; }   // bearish TDI -> SELL
   whyOut = "fast_eq_slow";
   return 0;
}
                        ...
                        const bool s1RequiredAgainstParent)
{
   ...
   if(parentVote == x)
   {
      if(s1Vote == x) return 1;                              // R1 aligned
      if(nOptional >= minWithParent) return 2;               // R2 parent carries
      return 0;
   }
   ...
   if(nAgainst >= minAgainstParent &&
      (!s1RequiredAgainstParent || s1Vote == x))
                          ...
                          bool &conflictOut)
{
   ruleOut = 0;
   conflictOut = false;
   const int rBuy  = XPDir_RulePassesFor(1, parentVote, s1Vote, optionalVotes,
                                         minWithParent, minAgainstParent,
                                         s1RequiredAgainstParent);
   const int rSell = XPDir_RulePassesFor(-1, parentVote, s1Vote, optionalVotes,
                                         minWithParent, minAgainstParent,
                                         s1RequiredAgainstParent);
...
bool XPDir_IsEmptyValue(const double v)
{
   return (!MathIsValidNumber(v) || v == EMPTY_VALUE || MathAbs(v) >= EMPTY_VALUE);
}
...
bool XPDir_Init()
{
   ...
   g_XPDirMaxRunLenBars = (InpDirMaxRunLenBars            < 0) ? 0 : InpDirMaxRunLenBars;
   if(g_XPDirMinWith != InpDirMinChildrenWithParent ||
      ...
      PrintFormat("XPDIR CLAMP min_with_parent=%d min_against_parent=%d max_stale_bars=%d",
   ...
   PrintFormat("XPDIR INIT mode=%s indicator=%s parent_tf=%s children=[S1%s%s%s%s%s] "
               ...
               "max_runlen_bars=%d max_stale_bars=%d csv=%d",
               ...
               g_XPDirMaxRunLenBars, g_XPDirMaxStaleBars, InpDirWriteCsv ? 1 : 0);
   g_XPDirReady = true;
   return true;
}
...
//+------------------------------------------------------------------+
//| One rung read: last CLOSED bar (index 1)                          |
//+------------------------------------------------------------------+
void XPDir_ReadRung(const int r)
{
   ...
   if(CopyBuffer(g_XPDirHandle[r], XPDIR_BUF_FAST, 1, 1, fastArr) != 1 ||
      CopyBuffer(g_XPDirHandle[r], XPDIR_BUF_SLOW, 1, 1, slowArr) != 1)
   { g_XPDirWhy[r] = "no_data"; return; }
   ...
   const double fast = fastArr[0];
   const double slow = slowArr[0];
   if(ArraySize(runArr) == 1 && !XPDir_IsEmptyValue(runArr[0]))
      g_XPDirRunLen[r] = (int)MathRound(runArr[0]);
   ...
   string why = "";
   g_XPDirVote[r] = XPDir_RungVote(fast, slow, g_XPDirRunLen[r], age,
                                   g_XPDirIntervalS[r], g_XPDirMaxStaleBars,
                                   g_XPDirMaxRunLenBars, why);
   if(why == "stale") why = StringFormat("stale_%I64ds", age);
   g_XPDirWhy[r] = why;
   if(g_XPDirVote[r] == 0) return;

   if(!g_XPDirReadyLogged[r])
   {
      g_XPDirReadyLogged[r] = true;
      PrintFormat("XPDIR RUNG_READY rung=%s symbol=%s bars=%d fast=%.4f slow=%.4f state=%s runlen=%d vote=%s",
                  g_XPDirTag[r], g_XPDirSymbol[r], Bars(g_XPDirSymbol[r], tf),
                  fast, slow,
                  XPDir_IsEmptyValue(g_XPDirState[r]) ? "-" : DoubleToString(g_XPDirState[r], 0),
                  g_XPDirRunLen[r], XPDir_VoteTag(r));
   }
}
...
ENUM_XPDIR XPDir_Current()
{
   ...
   const string line = StringFormat("dir=%s rule=%s P=%s C1=%s C5=%s C10=%s C15=%s C30=%s C45=%s runlen1=%d",
                                    ...
                                    XPDir_VoteTag(3), XPDir_VoteTag(4), XPDir_VoteTag(5),
                                    g_XPDirRunLen[XPDIR_IDX_S1]);
   if(line != g_XPDirLastStateLine)
   {
      g_XPDirLastStateLine = line;
      g_XPDirStateLines++;
...
void XPDir_PrintFunnel()
{
   ...
   for(int r = 0; r < XPDIR_RUNGS; r++)
   {
      ...
      PrintFormat("XPDIR FUNNEL rung=%s symbol=%s enabled=%d handle=%d bars=%d bar1_time=%s "
                  ...
                  XPDir_IsEmptyValue(g_XPDirState[r]) ? "-" : DoubleToString(g_XPDirState[r], 0),
                  g_XPDirRunLen[r], XPDir_VoteTag(r), g_XPDirWhy[r]);
   }
}
...
void XPDir_LogSent(const bool execIsBuy)
{
   ...
   PrintFormat("XPDIR DECISION mode=%s dir=%s rule=%s trigger_side=%s exec_side=%s action=SENT translated=%d",
