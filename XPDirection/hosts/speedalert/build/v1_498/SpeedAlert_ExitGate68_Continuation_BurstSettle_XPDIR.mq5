...
//+------------------------------------------------------------------+
#property strict
#property version   "1.498-XPDIR"
#property description "SpeedAlert continuation development candidate: enabled-by-default original-TP milestone, break-even and 2D trailing with coordinated exit freezing. | XPW direction veto (InpDirMode=DIR_OFF reproduces the unfiltered EA)."
...
void ManagePosition(const MqlTick &t)
{
   ...
   for(int i = PositionsTotal() - 1; i >= 0; i--)
   {
      ...
      if(eaExitsReleased && TrySpeedProfitExit(ticket, side, fav, idx))
         continue;
